...
class ExhibitionTest {
	...
	@Test
	void testSelectTickets() {
		Set<Ticket> ts = exhi.selectTickets(cli, LocalDateTime.of(LocalDate.now().plusDays(1), LocalTime.of(15, 0)), 3);
		assertEquals(ts.size(), 3);
		
		for(Ticket t : ts) {
			assertTrue(t instanceof Ticket);
		}
	}
	
	@Test
	void testCalculatePrice() {
		double pr = exhi.calculatePrice(cli, 3);
		assertEquals(pr, 30);
	}
